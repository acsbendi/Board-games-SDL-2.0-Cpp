...
void NineMensMorrisBoard::selectPoint(int point) {
    filledCircleRGBA(renderer,
                     static_cast<Sint16>(points[point - 1]->getCoordinates().x),
                     static_cast<Sint16>(points[point - 1]->getCoordinates().y), 25,
                     SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b, 255);
            ...
            case SDL_MOUSEBUTTONDOWN:
                if (event.button.button == SDL_BUTTON_LEFT)
                    for (point = 1; point < 25; point++)
                        if ((points[point - 1]->getCoordinates().x - event.button.x) *
                            (points[point - 1]->getCoordinates().x - event.button.x) +
                            (points[point - 1]->getCoordinates().y - event.button.y) *
                            (points[point - 1]->getCoordinates().y - event.button.y) < 20 * 20)
                            return;
...
void NineMensMorrisBoard::drawCantRemoveFrame(bool top) {
    ...
    showText(languageresourcemanager->getNineMensMorrisCantRemoveText1(),
             ...
             current == 0 ? 0 : FRAME_HEIGHT + BOARD_HEIGHT, current == 0 ? WHITE_LETTER_COLOR : BLACK_LETTER_COLOR,
             letterfont);
    showText(languageresourcemanager->getNineMensMorrisCantRemoveText2(),
             ...
             current == 0 ? REMOVE_TEXT_SPACE : FRAME_HEIGHT + BOARD_HEIGHT + REMOVE_TEXT_SPACE,
             current == 0 ? WHITE_LETTER_COLOR : BLACK_LETTER_COLOR, letterfont);
    SDL_RenderPresent(renderer);
    SDL_Delay(1000);
}
...
void NineMensMorrisBoard::setPoints() {
    ...
    */
    ...
    (*points[1])[2] = points[4];
    (*points[2])[0] = points[1];
    (*points[2])[1] = points[14];
    (*points[3])[0] = points[4];
    (*points[3])[1] = points[10];
    (*points[4])[0] = points[1];
    (*points[4])[1] = points[3];
    (*points[4])[2] = points[5];
    (*points[4])[3] = points[7];
    (*points[5])[0] = points[4];
    (*points[5])[1] = points[13];
    (*points[6])[0] = points[7];
    (*points[6])[1] = points[11];
    (*points[7])[0] = points[4];
    (*points[7])[1] = points[6];
    (*points[7])[2] = points[8];
    (*points[8])[0] = points[7];
    (*points[8])[1] = points[12];
    (*points[9])[0] = points[0];
    (*points[9])[1] = points[10];
    (*points[9])[2] = points[21];
    (*points[10])[0] = points[3];
    (*points[10])[1] = points[9];
    (*points[10])[2] = points[11];
    (*points[10])[3] = points[18];
    (*points[11])[0] = points[6];
    (*points[11])[1] = points[10];
    (*points[11])[2] = points[15];
    (*points[12])[0] = points[8];
    (*points[12])[1] = points[13];
    (*points[12])[2] = points[17];
    (*points[13])[0] = points[5];
    (*points[13])[1] = points[12];
    (*points[13])[2] = points[14];
    (*points[13])[3] = points[20];
    (*points[14])[0] = points[2];
    (*points[14])[1] = points[13];
    (*points[14])[2] = points[23];
    (*points[15])[0] = points[11];
    (*points[15])[1] = points[16];
    (*points[16])[0] = points[15];
    (*points[16])[1] = points[17];
    (*points[16])[2] = points[19];
    (*points[17])[0] = points[12];
    (*points[17])[1] = points[16];
    (*points[18])[0] = points[10];
    (*points[18])[1] = points[19];
    (*points[19])[0] = points[16];
    (*points[19])[1] = points[18];
    (*points[19])[2] = points[20];
    (*points[19])[3] = points[22];
    (*points[20])[0] = points[13];
    (*points[20])[1] = points[19];
    (*points[21])[0] = points[9];
    (*points[21])[1] = points[22];
    (*points[22])[0] = points[19];
    (*points[22])[1] = points[21];
    (*points[22])[2] = points[23];
    (*points[23])[0] = points[14];
    (*points[23])[1] = points[22];
...
void NineMensMorrisBoard::checkEnd() {
    ...
    for (int i = 0; i < 9; i++) {
        if ((*players[0])[i] != nullptr) count1++; //1st player's piece
        if ((*players[1])[i] != nullptr) count2++; //2nd player's piece
    }
...
bool NineMensMorrisBoard::anywhere(int player) {
    int counter = 0;
    for (int i = 0; i < 9; i++)
        if ((*players[player])[i] != nullptr) counter++;
    return counter <= 3;
}
...
void NineMensMorrisBoard::mill(const Point* point) {
    ...
    for (int i = 0; (i < (current == 0 ? numberofblackpieces : numberofwhitepieces)) && !removable; i++)
        if ((*players[current == 0 ? 1 : 0])[i] != nullptr && (*players[current == 0 ? 1 : 0])[i]->getPoint() != nullptr
            && checkMill((*players[current == 0 ? 1 : 0])[i]->getPoint()) < 0)
            removable = true;
    if (checkMill(point) >= 0 && removable) {
        ...
        while (!valid) {
            ...
            if (points[toberemoved - 1]->isOccupied() && checkMill(points[toberemoved - 1]) < 0 &&
                points[toberemoved - 1]->getPiece()->getOwner() == players[current == 0 ? 1 : 0]) {
                valid = true;
                for (int i = 0; i < (current == 0 ? numberofblackpieces : numberofwhitepieces); i++)
                    if ((*players[current == 0 ? 1 : 0])[i] != nullptr &&
                        (*players[current == 0 ? 1 : 0])[i]->getPoint()->getId() == toberemoved)
                        (*players[current == 0 ? 1 : 0])[i] = nullptr;
                delete points[toberemoved - 1]->getPiece();
                points[toberemoved - 1]->setPiece(nullptr);
                drawBoard();
            }
        }
...
int NineMensMorrisBoard::checkMill(const Point* point) {
    ...
    for (int i = 0; i < 16; i++)
        if (mills[i][0] == id || mills[i][1] == id || mills[i][2] == id) {
            ...
            for (int j = 0; j < 9; j++) {
                if ((*players[player])[j] != nullptr && (*players[player])[j]->getPoint() != nullptr) {
                    int id2 = (*players[player])[j]->getPoint()->getId();
                    if (id2 == mills[i][0] || id2 == mills[i][1] || id2 == mills[i][2]) count++;
                }
            }
            if (count == 3)
                return i; //mill found, since all there points along a line have the pieces of the same player
        }
    return -1; //no mills
}
...
void NineMensMorrisBoard::secondPhase() {
    ...
    while (!valid) {
        ...
        if (!anywherebool) {
            for (int i = 0; i < 9; i++)
                if ((*players[current])[i] != nullptr && (!((*(*players[current])[i]->getPoint())[0]->isOccupied()) ||
                                                          !((*(*players[current])[i]->getPoint())[1]->isOccupied()) ||
                                                          ((*(*players[current])[i]->getPoint())[2] != nullptr &&
                                                           !((*(*players[current])[i]->getPoint())[2]->isOccupied())) ||
                                                          ((*(*players[current])[i]->getPoint())[3] != nullptr &&
                                                           !((*(*players[current])[i]->getPoint())[3]->isOccupied()))))
            ...
            for (int i = 0; i < 9; i++)
                if ((*players[current])[i] != nullptr)
                    movable[numberofmovables++] = (*players[current])[i];
        }

        clickOnPoint(point1);
        if (points[point1 - 1]->getPiece() != nullptr &&
            find(movable, movable + 9, points[point1 - 1]->getPiece()) != movable + 9) {
            ...
            while (!valid) {
                ...
                if (!points[point2 - 1]->isOccupied() &&
                    (anywherebool || (*points[point2 - 1])[0] == points[point1 - 1] ||
                     (*points[point2 - 1])[1] == points[point1 - 1] ||
                     (*points[point2 - 1])[2] == points[point1 - 1] ||
                     (*points[point2 - 1])[3] == points[point1 - 1])) {
                    valid = true;
                    points[point2 - 1]->setPiece(mozgo);
                    mozgo->setPoint(points[point2 - 1]);
                    points[point1 - 1]->setPiece(nullptr);
                } else if (points[point2 - 1]->getPiece() != nullptr &&
                           find(movable, movable + 9, points[point2 - 1]->getPiece()) != movable + 9) {
                    point1 = point2;
                    drawBoard();
                }
            }
...
NineMensMorrisBoard::NineMensMorrisBoard(LanguageResourceManager* languageresourcemanager) {
    ...
    setPoints();
    initGraphics();
    players[0] = new Player(languageresourcemanager->getWhiteName(), 9, nullptr, "resources/images/white.png",
                            renderer);
    players[1] = new Player(languageresourcemanager->getBlackName(), 9, nullptr, "resources/images/black.png",
                            renderer);
    drawBoard();
...
void NineMensMorrisBoard::play() {
    while (!over) {
        ...
        drawFrame(current == 0);
        if (current == 0) {
            showText(languageresourcemanager->getWhiteName(), languageresourcemanager->getNineMensMorrisWhiteX(), 2,
                     WHITE_LETTER_COLOR, letterfont);
            showText(languageresourcemanager->getNineMensMorrisToMoveText(),
                     languageresourcemanager->getNineMensMorrisToMoveTextX(), 42, WHITE_LETTER_COLOR, letterfont);
        } else {
            showText(languageresourcemanager->getBlackName(), languageresourcemanager->getNineMensMorrisBlackX(), 502,
                     BLACK_LETTER_COLOR, letterfont);
            showText(languageresourcemanager->getNineMensMorrisToMoveText(),
                     languageresourcemanager->getNineMensMorrisToMoveTextX(), 542, BLACK_LETTER_COLOR, letterfont);
        }
...
void NineMensMorrisBoard::initGraphics() {
    ...
    window = SDL_CreateWindow(languageresourcemanager->getNineMensMorrisWindowTitle().c_str(), SDL_WINDOWPOS_UNDEFINED,
...
void NineMensMorrisBoard::showEnd(bool white) {
    ...
    rect = {150, 60, 100, 100};
    SDL_RenderCopy(renderer, cuptexture, nullptr, &rect);  //loading the cup on the end screen
    showText(players[white ? 0 : 1]->getName(),
             white ? languageresourcemanager->getNineMensMorrisEndScreenWhiteX()
                   : languageresourcemanager->getNineMensMorrisEndScreenBlackX(),
    ...
    SDL_RenderCopy(renderer, buttontexture, nullptr, &rect); //loading end button
    SDL_RenderPresent(renderer);
    SDL_Event event;
    while (SDL_WaitEvent(&event) && event.type != SDL_QUIT) {
        switch (event.type) {
