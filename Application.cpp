...
void Application::initGraphics() {
    ...
    if (languageselectionbottom == nullptr) {
        std::cerr << languageresourcemanager->getImageErrorText() << SDL_GetError() << std::endl;
        return;
    }

    window = SDL_CreateWindow(languageresourcemanager->getApplicationWindowTitle().c_str(),
...
void Application::noneSelected() {
    SDL_RenderCopy(renderer, startpicturetexture, nullptr, nullptr);
    SDL_Rect rect = {LANGUAGE_SELECTION_ICON_X, LANGUAGE_SELECTION_ICON_Y, LANGUAGE_SELECTION_ICON_SIDE,
                     LANGUAGE_SELECTION_ICON_SIDE};
    SDL_RenderCopy(renderer, languageselectionicontexture, nullptr, &rect);
}
...
void Application::nineMensMorrisSelected() {
    ...
    thickLineRGBA(renderer, 256, 0, 356, 480, 15, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    thickLineRGBA(renderer, 0, 5, 254, 5, 20, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    thickLineRGBA(renderer, 5, 11, 5, 480, 20, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    thickLineRGBA(renderer, 10, 475, 354, 475, 20, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    SDL_RenderPresent(renderer);
}
...
void Application::languageSelection() {
    ...
    SDL_RenderCopy(renderer, languageselectiontoptexture, nullptr, &rect);
    const vector<Language>& languages = languageresourcemanager->getLanguages();
    for (int i = 0; i < (int) languages.size(); i++) {
        rect = {LANGUAGE_LIST_X, LANGUAGE_LIST_Y + LANGUAGE_LIST_TOP_HEIGHT + i * LANGUAGE_LIST_ITEM_HEIGHT,
                LANGUAGE_LIST_WIDTH, LANGUAGE_LIST_ITEM_HEIGHT};
        SDL_RenderCopy(renderer, languages[i] == temporaryselectedlanguage ? languageselectionmiddlewhitetexture
                                                                           : languageselectionmiddletexture, nullptr,
                       &rect);
        SDL_FreeSurface(text);
        text = TTF_RenderUTF8_Blended(font, languageresourcemanager->getLanguageString(languages[i]).c_str(),
                                      LETTER_COLOR);
        SDL_DestroyTexture(texttexture);
        texttexture = SDL_CreateTextureFromSurface(renderer, text);
        rect = {LANGUAGE_LIST_TEXT_X, LANGUAGE_LIST_Y + LANGUAGE_LIST_TOP_HEIGHT + i * LANGUAGE_LIST_ITEM_HEIGHT -
                                      LANGUAGE_LIST_TEXT_Y_DIFFERENCE,
                LANGUAGE_LIST_TEXT_WIDTH, LANGUAGE_LIST_ITEM_HEIGHT
        };
        SDL_RenderCopy(renderer, texttexture, nullptr, &rect);
    }
    rect = {LANGUAGE_LIST_X,
            LANGUAGE_LIST_Y + LANGUAGE_LIST_TOP_HEIGHT + (int) languages.size() * LANGUAGE_LIST_ITEM_HEIGHT -
            LANGUAGE_LIST_BOTTOM_OVERLAP,
            LANGUAGE_LIST_WIDTH, LANGUAGE_LIST_BOTTOM_HEIGHT
    };
    SDL_RenderCopy(renderer, languageselectionbottomtexture, nullptr, &rect);
    SDL_RenderPresent(renderer);
...
void Application::snakesAndLaddersSelected() {
    ...
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    thickLineRGBA(renderer, 290, 10, 640, 10, 20, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    thickLineRGBA(renderer, 284, 0, 384, 480, 15, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    thickLineRGBA(renderer, 630, 15, 630, 480, 20, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    thickLineRGBA(renderer, 388, 470, 620, 470, 20, SELECTION_COLOR.r, SELECTION_COLOR.g, SELECTION_COLOR.b,
                  SELECTION_COLOR.a);
    SDL_RenderPresent(renderer);
}
