...
void SnakesAndLaddersBoard::roll(int number)
{
    ...
    while(click(x,y))
    {
        if( (ROLL_BUTTON_X + (double)ROLL_BUTTON_DIAMETER/2 - x)*(ROLL_BUTTON_X + (double)ROLL_BUTTON_DIAMETER/2 - x) +
                (ROLL_BUTTON_Y + (double)ROLL_BUTTON_DIAMETER/2 - y)*(ROLL_BUTTON_Y + (double)ROLL_BUTTON_DIAMETER/2 - y)  < ((double)ROLL_BUTTON_DIAMETER/2)*((double)ROLL_BUTTON_DIAMETER/2) )
        {
    ...
    */
    for(int i = 0; i < 10 ; i++)
    {
        for(int  j = 0; j < 10; j++)
        {
            ...
            {
            case 0:
                SDL_RenderCopy(renderer,lightsquaretexture,0,&rect);
                break;
            case 1:
                SDL_RenderCopy(renderer,darksquaretexture,0,&rect);
                break;
            case 2:
                SDL_RenderCopy(renderer,lightsquare2texture,0,&rect);
                break;
            case 3:
                SDL_RenderCopy(renderer,darksquare2texture,0,&rect);
                break;
            }
...
void SnakesAndLaddersBoard::turn()
{
    ...
    if(move->getId() == numberofpoints  || (*move)[0] == points[move->getId()])  //moved to the last square or to a normal square
    {
...
int SnakesAndLaddersBoard::selectNumberOfPlayers()
{
    ...
    while(click(x,y))
    {
        ...
        if(x > START_BUTTON_X && x < START_BUTTON_X + START_BUTTON_WIDTH && y > START_BUTTON_Y && y < START_BUTTON_Y + START_BUTTON_HEIGHT
                && !(x - START_BUTTON_X + y - START_BUTTON_Y < 6 ||  std::abs(x - START_BUTTON_X - START_BUTTON_WIDTH) + y - START_BUTTON_Y < 6
                     || x - START_BUTTON_X + std::abs(y - START_BUTTON_Y - START_BUTTON_HEIGHT) < 6  || std::abs(x - START_BUTTON_X - START_BUTTON_WIDTH) + std::abs(y - START_BUTTON_Y - START_BUTTON_HEIGHT) < 6) )
            break;
        //increment button
        if( x > INCREMENT_BUTTON_X && x < INCREMENT_BUTTON_X + INCREMENT_BUTTON_WIDTH && y > INCREMENT_BUTTON_Y && y < INCREMENT_BUTTON_Y + INCREMENT_BUTTON_HEIGHT &&
                ((x - INCREMENT_BUTTON_X < (double)INCREMENT_BUTTON_WIDTH/2 && y - INCREMENT_BUTTON_Y > INCREMENT_BUTTON_HEIGHT - (double)INCREMENT_BUTTON_HEIGHT/INCREMENT_BUTTON_WIDTH*(x-INCREMENT_BUTTON_X)*2 )
                 || ( x  - INCREMENT_BUTTON_X > (double)INCREMENT_BUTTON_WIDTH/2 && y - INCREMENT_BUTTON_Y > (double)INCREMENT_BUTTON_HEIGHT/INCREMENT_BUTTON_WIDTH*(x-INCREMENT_BUTTON_X)*2 - INCREMENT_BUTTON_HEIGHT) ) && temp < 6)
            drawSelection(++temp);
        //decrement button
        if( x > INCREMENT_BUTTON_X && x < INCREMENT_BUTTON_X + INCREMENT_BUTTON_WIDTH && y > DECREMENT_BUTTON_Y && y < DECREMENT_BUTTON_Y + INCREMENT_BUTTON_HEIGHT &&
                ((x - INCREMENT_BUTTON_X < (double)INCREMENT_BUTTON_WIDTH/2 && y - DECREMENT_BUTTON_Y < (double)INCREMENT_BUTTON_HEIGHT/INCREMENT_BUTTON_WIDTH*(x-INCREMENT_BUTTON_X)*2 )
                 || ( x  - INCREMENT_BUTTON_X > (double)INCREMENT_BUTTON_WIDTH/2 && y - DECREMENT_BUTTON_Y < INCREMENT_BUTTON_HEIGHT - (double)INCREMENT_BUTTON_HEIGHT/INCREMENT_BUTTON_WIDTH*(x-INCREMENT_BUTTON_X-(double)INCREMENT_BUTTON_WIDTH/2)*2) ) && temp > 1)
            drawSelection(--temp);
    }
    return temp;
}
...
{
    window = SDL_CreateWindow(languageresourcemanager->getSnakesAndLaddersWindowTitle().c_str(),SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN );
    if( window == NULL )
    {
...
void SnakesAndLaddersBoard::setPoints(int size, const char* connections)
{
    ...
    {
        (*points[i])[0] = points[i+1];        //by default, the point with an id greater by one comes after every point
    }
    (*points[numberofpoints-1])[0] = NULL;
    std::stringstream ss;
...
SnakesAndLaddersBoard::SnakesAndLaddersBoard(LanguageResourceManager* languageresourcemanager, int meret, const char* connections)
{
    ...
    {
    case 6:
        players[5] = new Player(languageresourcemanager->getGreenName(),1,points[0],"resources/images/greenpiece.png", renderer);
    case 5:
        players[4] = new Player(languageresourcemanager->getRedName(),1,points[0],"resources/images/redpiece.png", renderer);
    case 4:
        players[3] = new Player(languageresourcemanager->getPinkName(),1,points[0],"resources/images/pinkpiece.png", renderer);
    case 3:
        players[2] = new Player(languageresourcemanager->getBlueName(),1,points[0],"resources/images/bluepiece.png", renderer);
    case 2:
        players[1] = new Player(languageresourcemanager->getWhiteName(),1,points[0],"resources/images/whitepiece.png", renderer);
    case 1:
        players[0] = new Player(languageresourcemanager->getBlackName(),1,points[0],"resources/images/blackpiece.png", renderer);
        break;
    default:
        break;
    }
    current = numberofplayers - 1; //the current player changes BEFORE every move => we need to set it to last player here
